...
public class TypicalScenarioUserWantToGetAndCheckOffersIntegrationTest extends BaseIntegrationTest {
    ...
    public void user_has_to_be_authenticated_and_api_should_have_offers() throws Exception {
        ...
        // given && when
        ResultActions performRegistration = mockMvc.perform(post("/register")
                .content("""
                        {
                        "username": "someUser",
                        "password": "somePassword"
        ...
        assertAll(
                () -> assertThat(jwtResponseDto.username().equals("someUser")),
                () -> assertThat(jwtResponseDto.token().matches(String.valueOf(Pattern.compile("^([A-Za-z0-9-_=]+\\.)+([A-Za-z0-9-_=])+\\.?$")))
        ...
        wireMockServer.stubFor(WireMock
                ...
                .willReturn(WireMock.aResponse()
                        .withStatus(HttpStatus.OK.value())
                        .withHeader("Content-Type", "application/json")
                        .withBody(httpResponse.responseWithTwoObjects())
                )
        );
        ...
        //given && when
        MvcResult mvcResultWithTwoOffers = mockMvc.perform(get("/offers")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andReturn();
        ...
        //then
        assertAll(
                () -> assertThat(responseWithTwoOffers).hasSize(2),
                () -> assertThat(responseWithTwoOffers).containsExactlyInAnyOrder(
                        new OfferResponseDto(expectedFirstOffer.id(), expectedFirstOffer.company(), expectedFirstOffer.title(), expectedFirstOffer.salary(), expectedFirstOffer.offerUrl()),
                        new OfferResponseDto(expectedSecondOffer.id(), expectedSecondOffer.company(), expectedSecondOffer.title(), expectedSecondOffer.salary(), expectedSecondOffer.offerUrl())
                )
        );
        ...
        //then
        String successfulGetResultJson = getSingleofferById
                .andExpect(status()
                        .isOk())
                .andReturn()
                .getResponse()
        ...
        assertAll(
                () -> assertThat(offerResponseDto.id().equals(offerIdAddedToTheDatabase)),
                () -> assertThat(offerResponseDto.title().equals(offerTitleAddedToTheDatabase)),
                () -> assertThat(offerResponseDto.company().equals(offerCompanyAddedToTheDatabase)),
                () -> assertThat(offerResponseDto.salary().equals(offerSalaryAddedToTheDatabase)),
                () -> assertThat(offerResponseDto.offerUrl().equals(offerUrlAddedToTheDatabase))
        );
        ...
        // then
        assertAll(
                () -> assertThat(nextTwoUniqueOffers).hasSize(2),
                () -> assertThat(nextTwoUniqueOffers).containsExactlyInAnyOrder(
                        new OfferResponseDto(firstNewFetchedOffer.id(), firstNewFetchedOffer.company(), firstNewFetchedOffer.title(), firstNewFetchedOffer.salary(), firstNewFetchedOffer.offerUrl()),
                        new OfferResponseDto(secondNewFetchedOffer.id(), secondNewFetchedOffer.company(), secondNewFetchedOffer.title(), secondNewFetchedOffer.salary(), secondNewFetchedOffer.offerUrl())
                )
        );
        ...
        //then
        String jsonWithFourOffers = performWithFourOffers
                .andExpect(status()
                        .isOk())
                .andReturn()
                .getResponse()
        ...
        assertAll(
                () -> assertThat(responseWithFourOffers).hasSize(4),
                () -> assertThat(responseWithFourOffers).contains(
                        new OfferResponseDto(firstNewFetchedOffer.id(), firstNewFetchedOffer.company(), firstNewFetchedOffer.title(), firstNewFetchedOffer.salary(), firstNewFetchedOffer.offerUrl()),
                        new OfferResponseDto(secondNewFetchedOffer.id(), secondNewFetchedOffer.company(), secondNewFetchedOffer.title(), secondNewFetchedOffer.salary(), secondNewFetchedOffer.offerUrl())
                )
        ...
        assertAll(
                () -> assertThat(responseDto.id() != null),
